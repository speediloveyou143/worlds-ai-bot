...
const Interview = () => {
  ...
  const endInterview = () => {
    ...
    stopAllSpeech();
    stopMediaStreams();
    const finalMessage = `Great job! The interview is over. Your final score is ${score}/10.`;
  ...
  const stopInterview = () => {
    ...
    const finalMessage = `Alright, the interview has been stopped. Your score was ${score}/10.`;
    setConversation((prev) => {
  ...
  const handleStartInterview = (language) => {
  ...
  const startInterview = (language) => {
    setConversation([]);
    setQuestions(predefinedQuestions[language]);
    setCurrentQuestionIndex(0);
    setScore(0);
    setStatus(`Starting ${language} interview...`);
    setShowDashboard(false);

    const firstQuestion = predefinedQuestions[language][0];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 to-blue-950 flex  justify-center p-2 sm:p-8 h-full overflow-y-auto ">
      {isLoading && <Loader />}
      ...
      {(!selectedLanguage || showPermissionModal || showUserInfoModal || showApiKeyModal) && !showDashboard ? (
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="flex gap-4">
            {["Python", "JavaScript", "React"].map((lang) => (
              <button
                key={lang}
                onClick={() => handleStartInterview(lang)}
          ...
          {showDashboard ? (
            <div className="bg-gradient-to-br from-gray-900 to-blue-900 rounded-xl shadow-lg p-8 text-white mt-20 max-h-[85vh] sm:h-[550px] overflow-y-auto mb-[100px] px-2 sm:px-6">
              ...
              <div className="mb-6">
                ...
                <p className="text-lg">Score: <span className="font-bold">{score}/10</span></p>
