...
const FreeClass = () => {
  ...
  const [bootcampData, setBootcampData] = useState(null); // State to store bootcamp data
  const [loading, setLoading] = useState(true); // Loading state

  // Fetch bootcamp data based on id and set course in formData
  useEffect(() => {
    const fetchBootcamp = async () => {
      try {
        ...
        setFormData((prev) => ({
          ...prev,
          course: response.data.courseName, // Set course name from bootcamp data
        }));
        setLoading(false);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${BACKEND_URL}/create-register`, {
        ...
        bootcampId: id, // Optional: Include if your backend supports it
      });
      if (response.status === 200) {
        setShowRegistration(false); // Close the modal
  ...
  // Static data for Student Reviews
  const reviews = [
    {
      id: 1,
      name: "Sarah Chen",
