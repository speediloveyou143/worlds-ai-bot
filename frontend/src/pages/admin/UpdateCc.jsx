...
const UpdateCc = () => {
  ...
  useEffect(() => {
    const fetchCCertificates = async () => {
      try {
  ...
  const handleChange = (index, field, value) => {
    const updatedCertificates = [...cCertificates];
    updatedCertificates[index][field] = value;
    setCCertificates(updatedCertificates);
  };
  ...
  const handleRemoveCertificate = (index) => {
    const updatedCertificates = cCertificates.filter((_, i) => i !== index);
    setCCertificates(updatedCertificates);
  };
  ...
  const validate = () => {
    ...
    cCertificates.forEach((certificate, index) => {
      const fieldErrors = {};
  ...
  return (
    <div className="p-1 sm:p-6 sm:pb-0 pb-[110px] bg-gray-950 h-full overflow-y-auto m-4 rounded shadow-md">
      <h1 className="text-white text-xl font-bold mb-6">Update Course Certificates</h1>
      {message && (
        <p
      ...
      {cCertificates.map((certificate, index) => (
        <div
          key={index}
